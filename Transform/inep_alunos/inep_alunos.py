...
PATH = '/home/giuseppe/PycharmProjects/buscador_scripts/dados/'
...
def pega_arquivo_por_ano(ano):
    ...
    for arquivo_fs in os.listdir(path_ano):
        ...
        arquivo = open(arquivo_path, 'r')  # , encoding='latin-1')
...
def pega_cabecalho(ano):
    ...
    mais_cabecalho_ies = ['GEOGRAFICO', 'MANT_IES', 'id', 'ano_vigencia_inep','ano_grupo']
...
def adiciona_instituicao(ano, lista_alunos, dict_ies, cabecalho):
    ...
    for ALUNO in lista_alunos:
        ...
        ALUNO['ano_grupo'] = gYear(int(ano))

        # Converte de codigo para texto

        #ALUNO['CO_ESCOLARIDADE_ALUNO'] = ALUNO['CO'].decode('utf-8').encode('latin-1')
