...
"""
CHAVES_SIM_NAO = ['IN_CAPITAL_IES', 'IN_DEF_CEGUEIRA', 'IN_DEF_BAIXA_VISAO', 'IN_DEF_SURDEZ', 'IN_DEF_AUDITIVA',
                  'IN_DEF_FISICA', 'IN_DEF_SURDOCEGUEIRA', 'IN_DEF_MULTIPLA', 'IN_DEF_INTELECTUAL',
                  'IN_ATU_EAD', 'IN_ATU_POS_EAD', 'IN_ATU_EXTENSAO', 'IN_ATU_GESTAO', 'IN_ATU_GRAD_PRESENCIAL',
...
def adiciona_instituicao(ano, lista_docentes, dict_ies):
    ...
    for docente in lista_docentes:
        ...
        docente['CO_ESCOLARIDADE_DOCENTE'] = ESCOLARIDADE[docente['CO_ESCOLARIDADE_DOCENTE']].decode('utf-8').encode(
            'latin-1')
        docente['IN_DOCENTE_DEFICIENCIA'] = DEFICIENCIA_FISICA[docente['IN_DOCENTE_DEFICIENCIA']].decode(
            'utf-8').encode('latin-1')
        if 'IN_VISITANTE_IFES_VINCULO' in docente and docente['IN_VISITANTE_IFES_VINCULO']:
            docente['IN_VISITANTE_IFES_VINCULO'] = IN_VISITANTE_IFES_VINCULO[
                docente['IN_VISITANTE_IFES_VINCULO']].decode('utf-8').encode('latin-1')
